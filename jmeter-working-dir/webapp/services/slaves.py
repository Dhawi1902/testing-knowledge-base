...
def _provision_slave(ip: str, ssh_config: dict) -> dict:
    ...
    """
    steps = []
    status = {"java": False, "jmeter": False, "scripts": False, "firewall": False}
    slave_dir = ssh_config.get("slave_dir", "~/jmeter-slave")
    ...
    if "17" in out:
        steps.append({"name": "Java 17", "ok": True, "detail": "Already installed"})
        status["java"] = True
    else:
        out2, err2, code2 = run("sudo dnf install -y java-17-openjdk java-17-openjdk-devel 2>&1 | tail -3", timeout=180)
        if code2 == 0:
            steps.append({"name": "Java 17", "ok": True, "detail": "Installed"})
            status["java"] = True
        else:
            steps.append({"name": "Java 17", "ok": False, "detail": (err2 or out2)[:200]})
    ...
    if "firewall_configured" in out:
        steps.append({"name": "Firewall", "ok": True, "detail": "Ports 1099, 50000-50100 opened"})
