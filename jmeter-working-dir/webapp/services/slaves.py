...
def _provision_slave(ip: str, ssh_config: dict) -> dict:
    """Run idempotent provisioning on a single slave via SSH (#17, #20).

    Steps: Java 17, JMeter 5.6.3, dirs, scripts (always refresh), firewall.
    Returns {ip, ok, steps: [{name, ok, detail}], status: {java, jmeter, scripts, firewall}}.
    """
    steps = []
    ...
    try:
        ...
        steps.append({"name": "SSH Connect", "ok": False, "detail": str(e)})
    ...
    if "21" in out:
        steps.append({"name": "Java 21", "ok": True, "detail": "Already installed"})
        status["java"] = True
    else:
        out2, err2, code2 = run("sudo dnf install -y java-21-openjdk java-21-openjdk-devel 2>&1 | tail -3", timeout=180)
        if code2 == 0:
            steps.append({"name": "Java 21", "ok": True, "detail": "Installed"})
            status["java"] = True
        else:
            steps.append({"name": "Java 21", "ok": False, "detail": (err2 or out2)[:200]})

    # 2. Install JMeter 5.6.3
    out, err, code = run("test -f /opt/jmeter/bin/jmeter && echo ok")
    if "ok" in out:
        steps.append({"name": "JMeter 5.6.3", "ok": True, "detail": "Already installed at /opt/jmeter"})
        status["jmeter"] = True
    else:
        jmeter_cmds = (
            "cd /opt && "
        ...
        if "installed" in out2:
            steps.append({"name": "JMeter 5.6.3", "ok": True, "detail": "Downloaded and installed"})
            status["jmeter"] = True
        else:
            steps.append({"name": "JMeter 5.6.3", "ok": False, "detail": (err2 or out2)[:200]})

    # 3. Create directories
    out, err, code = run(f"mkdir -p {slave_dir}/test_data && echo ok")
    if "ok" in out:
        steps.append({"name": "Directories", "ok": True, "detail": f"{slave_dir}/test_data/"})
    else:
        steps.append({"name": "Directories", "ok": False, "detail": (err or out)[:200]})
    ...
    if "ok" in out and "ok" in out2:
        steps.append({"name": "Scripts (refreshed)", "ok": True, "detail": "start-slave.sh + stop-slave.sh written"})
        status["scripts"] = True
    else:
        steps.append({"name": "Scripts", "ok": False, "detail": ((err or out) + " " + (err2 or out2))[:200]})
    ...
    if agent_content:
        ...
        if "ok" in out and "svc_ok" in out2:
            steps.append({"name": "Metrics Agent", "ok": True, "detail": f"Deployed on port {agent_port}"})
            status["agent"] = True
        else:
            steps.append({"name": "Metrics Agent", "ok": False, "detail": ((err or out) + " " + (err2 or out2))[:200]})
    else:
        steps.append({"name": "Metrics Agent", "ok": False, "detail": "Agent script not found locally"})
    ...
    if "firewall_configured" in out:
        steps.append({"name": "Firewall", "ok": True, "detail": f"Ports 1099, 50000-50100, {agent_port} opened"})
        status["firewall"] = True
    elif "no_firewalld" in out:
        steps.append({"name": "Firewall", "ok": True, "detail": "firewalld not running (OCI security list handles it)"})
        status["firewall"] = True
    else:
        steps.append({"name": "Firewall", "ok": False, "detail": (err or out)[:200]})
    ...
    if code == 0:
        steps.append({"name": "Environment", "ok": True, "detail": "JAVA_HOME + JMETER_HOME in .bashrc"})
    else:
        steps.append({"name": "Environment", "ok": False, "detail": (err or out)[:200]})
...
async def provision_slave(ip: str, ssh_config: dict) -> dict:
    """Async wrapper for provisioning."""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(_executor, _provision_slave, ip, ssh_config)
